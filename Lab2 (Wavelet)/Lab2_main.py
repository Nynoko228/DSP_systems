...
if __name__ == "__main__":
    ...
    lvls = 4
    # approximation, details = schitaem_urovni(lvls, signal)

    # for i in range(lvls):
    #     plt.subplot(lvls, 2, i * 2 + 1)
    ...
    for j in txt_files:
        ...
        lst, cnt = read_from_file(j)
        # print(lst, cnt)
        cnt, L = is_power_of_two(cnt)
        print(L, cnt)
        approximation, details = schitaem_urovni(L, lst[:cnt])
        fs = txt_files[j]
        for i in range(L):
